...
let ControllerRegistry = class ControllerRegistry {
    constructor(license, authService, globalConfig, metadata) {
    ...
    activateController(app, controllerClass) {
        ...
        for (const [handlerName, route] of metadata.routes) {
            ...
            router[route.method](route.path, ...(backend_common_1.inProduction && route.rateLimit
                ...
                : []), ...(route.skipAuth ? [] : [this.authService.authMiddleware]), ...(route.licenseFeature ? [this.createLicenseMiddleware(route.licenseFeature)] : []), ...(route.accessScope ? [this.createScopedMiddleware(route.accessScope)] : []), ...controllerMiddlewares, ...route.middlewares, route.usesTemplates
                ? async (req, res) => {
