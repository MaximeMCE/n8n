...
let ProjectService = class ProjectService {
    constructor(sharedWorkflowRepository, projectRepository, projectRelationRepository, roleService, sharedCredentialsRepository, cacheService, licenseState, databaseConfig) {
    ...
    async addUsersToProject(projectId, relations) {
        ...
        await this.projectRelationRepository.save(relations.map((relation) => ({ projectId, ...relation })));
    }
    ...
    isUserProjectOwner(project, userId) {
        return project.projectRelations.some((pr) => pr.userId === userId && pr.role === 'project:personalOwner');
    }
