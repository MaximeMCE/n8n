...
let WorkflowRunner = class WorkflowRunner {
    constructor(logger, errorReporter, activeExecutions, executionRepository, workflowStaticDataService, nodeTypes, credentialsPermissionChecker, instanceSettings, manualExecutionService, executionDataService, eventService) {
        this.logger = logger;
    ...
    async run(data, loadStaticData, realtime, restartExecutionId, responsePromise) {
        ...
        try {
            ...
            const runData = this.executionDataService.generateFailedExecutionFromError(data.executionMode, error, error.node);
            const lifecycleHooks = (0, execution_lifecycle_hooks_1.getLifecycleHooksForRegularMain)(data, executionId);
        ...
        const shouldEnqueue = process.env.OFFLOAD_MANUAL_EXECUTIONS_TO_WORKERS === 'true'
    ...
    async enqueueExecution(executionId, workflowId, data, loadStaticData, realtime) {
        ...
        try {
            ...
            lifecycleHooks = (0, execution_lifecycle_hooks_1.getLifecycleHooksForScalingMain)(data, executionId);
            await lifecycleHooks.runHook('workflowExecuteBefore', [undefined, data.executionData]);
        }
        catch (error) {
            const lifecycleHooks = (0, execution_lifecycle_hooks_1.getLifecycleHooksForScalingWorker)(data, executionId);
