...
const findOwnerProject = (owner, accessibleProjects) => {
    if (typeof owner === 'string') {
        return accessibleProjects.find((project) => project.projectRelations.some((r) => r.role === 'project:personalOwner' && r.user.email === owner));
    }
    if (owner.type === 'personal') {
        return accessibleProjects.find((project) => project.type === 'personal' &&
            project.projectRelations.some((r) => r.role === 'project:personalOwner' && r.user.email === owner.personalEmail));
    }
...
const getOwnerFromProject = (remoteOwnerProject) => {
    ...
    if (remoteOwnerProject?.type === 'personal') {
        const personalEmail = remoteOwnerProject.projectRelations?.find((r) => r.role === 'project:personalOwner')?.user?.email;
        if (personalEmail) {
            owner = {
                type: 'personal',
...
let SourceControlImportService = class SourceControlImportService {
    ...
    async getRemoteVersionIdsFromFiles(context) {
        ...
        const accessibleProjects = await this.sourceControlScopedService.getAdminProjectsFromContext(context);
        const remoteWorkflowsRead = await Promise.all(remoteWorkflowFiles.map(async (file) => {
        ...
        const remoteWorkflowFilesParsed = remoteWorkflowsRead
            ...
            return (context.hasAccessToAllProjects() || findOwnerProject(remote.owner, accessibleProjects));
        })
            .map((remote) => {
