...
let ManualExecutionService = class ManualExecutionService {
    ...
    getExecutionStartNode(data, workflow) {
        ...
        const manualTrigger = workflow
            .getTriggerNodes()
            .find((node) => node.type === n8n_workflow_1.MANUAL_TRIGGER_NODE_TYPE);
    ...
    runManually(data, workflow, additionalData, executionId, pinData) {
        if (data.triggerToStartFrom?.data && data.startNodes?.length) {
            ...
            if (data.destinationNode !== data.triggerToStartFrom.name) {
                const recreatedStack = (0, n8n_core_1.recreateNodeExecutionStack)((0, n8n_core_1.filterDisabledNodes)(n8n_core_1.DirectedGraph.fromWorkflow(workflow)), new Set(startNodes), runData, data.pinData ?? {});
            ...
            if (data.destinationNode) {
                ...
                a.ok(destinationNode, `Could not find a node named "${data.destinationNode}" in the workflow.`);
                const destinationNodeType = workflow.nodeTypes.getByNameAndVersion(destinationNode.type, destinationNode.typeVersion);
                if (n8n_workflow_1.NodeHelpers.isTool(destinationNodeType.description, destinationNode.parameters)) {
                    const graph = (0, n8n_core_1.rewireGraph)(destinationNode, n8n_core_1.DirectedGraph.fromWorkflow(workflow), data.agentRequest);
            ...
            return workflowExecute.run(workflow, startNode, data.destinationNode, data.pinData, data.triggerToStartFrom);
        }
        else {
            this.logger.debug(`Execution ID ${executionId} is a partial execution.`, { executionId });
            const workflowExecute = new n8n_core_1.WorkflowExecute(additionalData, data.executionMode);
            if (data.partialExecutionVersion === 2) {
                return workflowExecute.runPartialWorkflow2(workflow, data.runData, data.pinData, data.dirtyNodeNames, data.destinationNode, data.agentRequest);
            }
            else {
