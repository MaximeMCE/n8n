...
let TestRunnerService = TestRunnerService_1 = class TestRunnerService {
    ...
    validateEvaluationTriggerNode(workflow) {
        ...
        if (!triggerNode.credentials ||
            ...
            throw new errors_ee_1.TestRunError('EVALUATION_TRIGGER_NOT_CONFIGURED', { node_name: triggerNode.name });
        }
    ...
    validateSetMetricsNodes(workflow) {
        ...
        const unconfiguredMetricsNode = metricsNodes.find((node) => !node.parameters ||
    ...
    validateSetOutputsNodes(workflow) {
        ...
        if (setOutputsNodes.length === 0) {
            throw new errors_ee_1.TestRunError('SET_OUTPUTS_NODE_NOT_FOUND');
        }
    ...
    static getEvaluationMetricsNodes(workflow) {
        return workflow.nodes.filter((node) => node.type === n8n_workflow_1.EVALUATION_NODE_TYPE && node.parameters.operation === 'setMetrics');
    }
