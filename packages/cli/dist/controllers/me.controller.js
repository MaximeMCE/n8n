...
let MeController = class MeController {
    ...
    async updateCurrentUser(req, res, payload) {
        const { id: userId, email: currentEmail, mfaEnabled, firstName: currentFirstName, lastName: currentLastName, } = req.user;
        ...
        if (((0, sso_helpers_1.isLdapCurrentAuthenticationMethod)() || (0, sso_helpers_1.isOidcCurrentAuthenticationMethod)()) &&
        ...
        const fieldsChanged = changeableFields.filter((key) => key in payload && payload[key] !== preUpdateUser[key]);
    ...
    async updatePassword(req, res, payload) {
        ...
        if (!passwordValidation.success) {
            throw new bad_request_error_1.BadRequestError(passwordValidation.error.errors.map(({ message }) => message).join(' '));
        }
        if (user.mfaEnabled) {
    ...
    async storeSurveyAnswers(req) {
        ...
        if (!personalizationAnswers) {
            this.logger.debug('Request to store user personalization survey failed because of undefined payload', {
                userId: req.user.id,
            });
            throw new bad_request_error_1.BadRequestError('Personalization answers are mandatory');
        }
        const validatedAnswers = (0, class_transformer_1.plainToInstance)(survey_answers_dto_1.PersonalizationSurveyAnswersV4, personalizationAnswers, { excludeExtraneousValues: true });
